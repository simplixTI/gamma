...
const RideHistory = () => {
  ...
  useEffect(() => {
    const fetchRides = async () => {
      // Use user.id as primary identifier, fallback to deviceId
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalSpent: completed.reduce((sum, r) => sum + Number(r.price), 0),
    };
  }, [filteredRides]);

  const handleExport = () => {
    if (filteredRides.length === 0) {
      toast.error('Nenhuma viagem para exportar');
      return;
    }
    exportRidesToCSV(filteredRides, 'minhas-viagens');
    ...
    <div className="min-h-screen min-h-[100dvh] bg-background">
      ...
      <header className="bg-primary text-primary-foreground safe-area-top">
        <div className="flex items-center gap-3 p-4">
          ...
          <h1 className="text-lg font-bold">Histórico de Viagens</h1>
        </div>
        ...
        <div className="flex items-center justify-around pb-4 px-4">
          <div className="text-center">
            ...
            <p className="text-xs opacity-70">Viagens</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <h2 className="font-semibold text-foreground mb-2">
              {rides.length === 0 ? 'Nenhuma viagem ainda' : 'Nenhuma viagem encontrada'}
            </h2>
            <p className="text-sm text-muted">
              {rides.length === 0 ? 'Suas viagens aparecerão aqui' : 'Tente ajustar os filtros'}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              <div key={ride.id} className="bg-card rounded-xl p-4 shadow-sm">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-foreground">R${Number(ride.price).toFixed(0)}</p>
                    {ride.pilot_name && (
                      <p className="text-xs text-muted mt-1">{ride.pilot_name}</p>
                    )}
                  </div>
                </div>
              </div>
